...
One of the examples Giacomo uses is a test for Gleam's LSP which looks like:

![Gleam Snapshot before](gleam-snapshot-before.png)

This is how such a test looks like for the gleam compiler. To be fair it's pretty readable and even without seeing the test you can figure out what's beeing tested here. However, the thing that keeps bothering me is that there is no syntax highlighting and that the very same LSP in my IDE doesn't recognize this file as Gleam.
And even if I force my editor to interpret the file as Gleam it won't work perfectly because the additional frontmatter at the top and the diagnostic output at the bottom doesn't make it valid syntax. 
